...
abstract_split <- initial_split(abstract_data, strata = activity)
abstract_train <- training(abstract_split)
abstract_test <- testing(abstract_split)

abstract_rec <- recipe(institute ~ word, data = abstract_train) |>
  step_tokenize(word) |>
  step_stopwords(word) |>
  step_tokenfilter(word, max_tokens = 500) |>
  step_tfidf(word) |>
...
set.seed(123)
abstract_folds <- bootstraps(abstract_train, strata = activity)
abstract_folds
```
