...
We also provide several precomputed variables and tables that enable quick and easy exploratory analysis:
  ...
  * `project_output`: This table contains pre-computed values for overall proeject cost (`project.cost`), as well as the number of publications (`n.pubs`) and patents (`n.patents`).
...
```{r grant_pubs, message = FALSE, warning=FALSE}
