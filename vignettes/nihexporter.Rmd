...
```{r cost, fig.height=4, fig.show='animate', fig.width=7, message=FALSE}
library(nihexporter)
library(dplyr)
library(ggplot2)
library(gganimate)
...
p <- gp + geom_point(size = 5)
 
gganimate(p)
```

It is also helpful to view these numbers as trajectories, to see the path of funding over time.

```{r cost_trajectories, message=FALSE, fig.width=7, fig.height=4, fig.show='animate'} 
