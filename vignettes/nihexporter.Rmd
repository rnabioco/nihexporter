...
cost.over.time <- projects |>
  ...
  group_by(fiscal_year, institute) |>
  summarize(yearly_cost = sum(fy_cost, na.rm = TRUE))
...
institution_funding <- projects |>
  filter(activity == 'R01') |>
  group_by(org_name, org_duns, fiscal_year) |>
  summarise(total_dollars = sum(fy_cost, na.rm = TRUE)) |>
  arrange(desc(total_dollars))

institution_funding |>
  select(fiscal_year, org_name, org_duns, total_dollars) |>
  filter(!is.na(org_duns)) |>
  mutate(total_award_millions = total_dollars / 1e6) |>
...
dollars_per_pi <- projects |>
  ...
  unique() |>
  group_by(pi_id) |>
  summarise(pi_dollars = sum(total_cost) / 1e6) |>
  arrange(desc(pi_dollars))  
...
high_perf_not_r01 <- projects |>
  ...
  mutate(cost_per_pub = total_cost / n_pubs) |>
  arrange(cost_per_pub)

high_perf_not_r01 |>
  head(10) |>
  mutate(total_cost = round(total_cost / 1e6, 3),
         cost_per_pub = comma(round(cost_per_pub, 0))) |>
...
productive_pis <- projects |>
  ...
  left_join(project_pis, by = 'project_num') |>
  group_by(pi_id) |>
  summarize(total_pi_dollars = sum(total_cost, na.rm = TRUE),
            total_pubs = sum(n_pubs)) |>
  mutate(pub_cost = total_pi_dollars / total_pubs) |>
...
productive_pis_all_time <- projects |>
  ...
  select(pi_id, activity, pub_cost, patent_cost, 
...
dollar_bin_grant_costs <- grant_costs |>
  group_by(institute) |>
  mutate(dollar.tile = findInterval(total_cost, vec = breaks, 
...
dollar_bin_grant_costs |>
  ...
  ggplot(aes(x = factor(dollar.tile), y = n_pubs)) +
...
ggplot(inst_costs, aes(x = factor(fiscal_year),
                       y = fy_cost, 
                       fill = activity)) +
...
research_costs <- grant_costs |>
  ...
  group_by(project_num, institute, fiscal_year) |>
  summarize(project_cost = sum(fy_cost, na.rm = TRUE)) |>
...
ggplot(combined_tbl, aes(x = factor(fiscal_year),
                         y = project_cost,
                         fill = type)) +
  geom_boxplot(outlier.shape = NA) + 
  scale_y_log10(limits = c(1e4,1e7),
...
```{r grant_stamina, eval=FALSE}
long_grants <- projects |>
  filter(activity == 'R01') |>
  select(project_num, project_start, project_end) |>
  group_by(project_num) |>
  # use lubridate here
  summarize(longest_run = max(project_end) - min(project_start)) |>
  arrange(desc(longest_run)) |>
  mutate(in_years = as.numeric(longest_run) / 365) |>
