...
Here are some summary stats on that table, with PI counts, grant counts and median grant cost.
...
grant_costs %>%
  left_join(project.pis) %>%
    group_by(pi.id) %>%
      summarise(n.pis)
  group_by(institute) %>%
  summarize(median.cost = median(lifetime.cost),
            n.grants = n())
```
...
Now we can make plots reminiscent of Berg's original.

```{r plot.productivity, message = FALSE, warning=FALSE}
grant_productivity %>%
...
```{r plot.dollar.bins, warning = FALSE, message = FALSE}
